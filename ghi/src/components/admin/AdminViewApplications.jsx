...
`;
	...
	const handleDeleteAndSendEmailAndUpdateRole = async (
		appId,
		email,
		uid
	) => {
		...
		if (selectedApplication) {
			switch (actionType) {
				...
				case "deleteAndSendEmailAndUpdateRole":
					handleDeleteAndSendEmailAndUpdateRole(
						selectedApplication.id,
						selectedApplication.email,
	...
	const renderModal = () => {
		...
		const message = {
			...
			sendAcceptanceEmail:
				"**EMAIL WARNING** Confirm sending acceptance email?",
			...
			deleteAndSendEmail:
				"**EMAIL WARNING** Confirm application deletion and sending of email? This Will Remove All Data Associated With This Application.",
			deleteAndSendEmailAndUpdateRole:
				"**EMAIL WARNING** Confirm application deletion and sending of email? This Will Remove All Data Associated With This Application. This will also automatically delete the user in 24 hours.",
			sendApplicationByEmail:
				"**SAFE** Confirm sending of application details email to FJZ?",
		};
			...
			<div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
				<div className="bg-white p-4 rounded shadow-md">
					...
					<div className="flex justify-end mt-4">
						<button
							className="bg-green-500 m-2 p-2 text-white rounded-lg hover:bg-green-900"
							onClick={executeAction}
						>
							Confirm
						</button>
						<button
							className="bg-red-500 m-2 p-2 text-white rounded-lg hover:bg-red-900"
							onClick={closeModal}
						>
							Cancel
						</button>
				...
				{orderList.map((key, index) => {
					...
					if (value && value !== "") {
						return (
							<div
								key={index}
								className="mb-4 last:mb-0 p-4 bg-gray-100 dark:bg-gray-900 rounded-md"
							>
								<h4 className="mb-2 font-semibold text-lg dark:text-gray-300">
									{key}:
								</h4>
								{[
									"ID Proof",
				...
				<div className="space-y-4 pt-10">
					...
					<div className="flex items-center space-x-4">
						<button
							...
							onClick={() => openModal("sendAcceptanceEmail")}
						>
					...
					<div className="flex items-center space-x-4">
						<button
							...
							onClick={() => openModal("deleteAndSendEmail")}
						>
						...
						<button
							...
							onClick={() =>
								openModal("deleteAndSendEmailAndUpdateRole")
							}
						>
					...
					<div className="flex items-center space-x-4">
						<button
							...
							onClick={() => openModal("sendApplicationByEmail")}
						>
	...
	return (
		<div className="flex flex-col md:flex-row md:space-x-6 space-y-6 md:space-y-0">
						...
						>
							...
							application["Last Name"] ? (
								<>
									...
									<span className="block dark:text-gray-400 text-lg">
			...
			<div className="w-full md:w-2/3 p-6 bg-white rounded shadow-lg">
				{selectedApplication ? (
					renderDetails(selectedApplication)
				) : (
